/*
SGDB: MySQL
Script: Políticas de BLOQUEO a nivel TABLA
*/
...
-- Bloqueo de las tablas mientras se carga la información 
-- Evita que otras transacciones afecten a los datos
/*
ministerio WRITE: Bloquea la tabla ministerio en modo escritura. 
Esto significa que otros usuarios no podrán realizar lecturas ni escrituras en esa tabla mientras dure el bloqueo. 
Este tipo de bloqueo garantiza que la tabla pueda ser modificada por la sesión que ejecutó la sentencia, 
pero previene que otras sesiones modifiquen o lean de ella durante el bloqueo.

miembro READ: Bloquea la tabla miembro en modo lectura. 
Esto permite que otros usuarios lean de la tabla, pero no puedan modificarla (es decir, no pueden hacer operaciones de escritura). 
*/
LOCK TABLES ministerio WRITE, miembro READ;
...
-- Desactiva el bloqueo transaccional que existía a nivel de tabla.
UNLOCK TABLES;
