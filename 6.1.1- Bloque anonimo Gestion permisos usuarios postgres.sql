...
BEGIN -- Inicia el bloque de código ejecutable, donde se colocan las instrucciones que se van a ejecutar
    ...
    FROM T_USUARIO u
    	JOIN T_USUARIO_ROL ur
			ON u.identificador = ur.id_usuario -- Une la tabla de usuarios con la tabla intermedia
   		JOIN T_ROL r
			ON ur.id_rol = r.identificador -- Une la tabla intermedia con la tabla de roles
